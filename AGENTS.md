...
- **Session kickoff is a task with a synthesized title, and per-task overrides beat defaults.** `POST /api/sessions` ([app/api/sessions/route.ts](app/api/sessions/route.ts)) is the one-call create+dispatch door behind the dashboard's **New session** composer ([app/session/new/page.tsx](app/session/new/page.tsx)). It funnels through `createTask` + `runTaskWithCap` — never a parallel pipeline. Three `tasks` columns (in BOTH `schema.sql` and `ensure()`): `model`, `autonomy` (NULL = inherit), `source` (`'manual'`|`'session'`, audit trail). Precedence, tested in `dispatch.test.ts`: **model** — a `task.model` override skips routing rules entirely (it's an explicit choice of model AND provider, so a rule can't flip the session onto a provider that drops the model); without one, routing-rule match → provider default. **autonomy** = `task.autonomy` → `project.autonomy`, and the route rejects an autonomy override for non-claude providers (gemini can't enforce it — the transcript must never claim a profile that isn't real). Follow-ups inherit both (no silent escalation between turns). The route also layers: an in-process token-bucket rate limit ([lib/api/rate-limit.ts](lib/api/rate-limit.ts), burst 10 / 10 per min), a 20k-char prompt cap, a DD-017 path preflight (409 naming the stale path), and the optional `DRYDOCK_LOCAL_DISPATCH_ONLY=1` guard ([lib/api/local-only.ts](lib/api/local-only.ts)) that rejects non-local dispatch on `/api/sessions`, `/run`, and `/followup` while leaving view/stream/cancel remote-capable. Security model: [docs/setup.md](docs/setup.md) §12.
...
- **There is exactly ONE Claude JSONL walker for the ledger.** [lib/connectors/claude-scan.ts](lib/connectors/claude-scan.ts) emits usage rows, session boundaries, and `pr-link` records in a single pass over the same 1.3 GB. A second walker would double the cold-read cost of the most expensive thing DryDock reads and let the two drift on what counts as a turn. Need a third thing? Add it to `ScanResult`. (`lib/providers/claude-usage.ts` is untouched and answers a different question — rolling 5h/weekly windows at a granularity days can't express.)
  - **Scope of that rule: anything that counts.** It binds every reader that produces usage, turns, sessions-per-day or `pr-link` data, because those numbers have to agree. [lib/connectors/recent-sessions.ts](lib/connectors/recent-sessions.ts) deliberately sits outside it and **must never start counting** — it answers *what was I doing*, not *how much*, and derives no figure the ledger also derives. Codex flagged it against this rule on PR #41; the reason it isn't folded into `ScanResult` is that doing so would break the invariant two bullets down: the recent-session read is on the dashboard's first paint, and `claude-scan` is the 1.3 GB full-history pass whose first run took 83 seconds. Folding them turns a bounded ~250 ms read (mtime pre-filter, then a 32 KB head + 512 KB tail per file, ≤ 40 files) into a blocking full scan. Two invariants pull opposite ways here and this is the documented resolution: **bounded, display-only, never a number.** If you ever need a *count* out of those logs, it goes in `ScanResult`.
...
app/
  ...
  project/[id]/page.tsx         # Project detail + ProjectDocs reader; opens the
                                # StreamViewer for a one-shot ?stream=<taskId> param
  session/new/page.tsx          # New-session composer: project picker + prompt +
                                # provider + <details> Advanced (model/autonomy)
  ...
  api/
    ...
    sessions/route.ts           # POST one-call kickoff: create task (source='session',
                                # synthesized title, model/autonomy overrides) + dispatch.
                                # Rate-limited; DD-017 path preflight; local-only guard.
...
- **Distinguish "unread" from "zero" in every payload and every tile.** This codebase has produced the same bug six times (see the named bug class in [CLAUDE.md](CLAUDE.md)). Concretely: a `status` field alongside the data, never a bare empty array; an em dash rather than `0` when a read failed; `filesScanned` / `unreadableRepos` counts so a partial read can't pass as a total; and a reason string the UI can render.
- **A connector that runs more than one query needs availability per *count*, not one `status` for the whole read.** Both GitHub connectors return `status: "ok"` when either half succeeds — by design, so one failure doesn't blank the other — which means `status` alone cannot distinguish "no open issues" from "the issues query failed". Every count carries its own flag (`pullsOk`/`issuesOk`, `mergedOk`/`openedOk`) and becomes `null` when unread; `null` renders as an em dash. Codex found four instances of this on PR #37, in code written an hour after the rule above was added to this file. Reference shapes: [lib/insights/overview.ts](lib/insights/overview.ts), [lib/connectors/github-work.ts](lib/connectors/github-work.ts).
- **Never add a nullable count to a non-nullable one.** A derived total (`openPulls + openIssues`) must be `null` unless every input was read, or the surviving half gets reported as the sum.
- **Render a count that hit its query limit as `N+`.** `gh search` caps each category at 30 (`SEARCH_LIMIT`), so a category on exactly 30 is an unknown backlog. Compare against the raw row count, not the mapped list — a row dropped in mapping still consumed a slot.
- **Stale-while-revalidate needs its client half.** A route that answers stale with `refreshing: true` obliges the caller to poll back for the rebuilt payload; a one-shot `useEffect` leaves that browser on old numbers under a permanent "refreshing" label. Bounded follow-up poll, cleared on unmount — see [components/OpenerOverview.tsx](components/OpenerOverview.tsx).
